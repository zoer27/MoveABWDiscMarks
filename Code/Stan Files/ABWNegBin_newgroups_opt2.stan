...
model {
  ...
  real q; //q for Matsouka abundance, only one for option 2
  ...
  //model predictions
   ...
   //}
    ...
    //qs for Mat abundance estimates --from MLE
      q = exp(mean(log(AEstMatInd./PredAbundMatInd))); //Indian
      //q[2] = exp(mean(log(AEstMatPac./PredAbundMatPac))); //Pacific
      
      //q[1] = fmax(q[1], 0.0001);
      //print(q);
      q = fmin(q, 1);//if q > 1, brings it back down to 1 -should make this not fit the data well and move away from this space
      //q[2] = fmin(q[2], 1);
      //print(q);
    //lognormal likelihood--Matsouka
    AEstMatInd ~ lognormal(log(q * PredAbundMatInd), sig[IndCVIdx]); //Indian not including last one
...
generated quantities{
  ...
  real q; //q parameter for Matsouka abundance estimates
  ...
  vector[ABr+AMat +((Nyeartag -1)*Nbasin*Nbasin)] log_like; //vector of loglikelihood values for model selection, Rec values are created in a matrix and then returned in column-major order
   {
    vector[ABr] PredAbundBr; //vector for subsetting population predictions (Branch)
    vector[AMat] PredAbundMatInd; //vector for subsetting population predictions, Indian (Matsouka)
    //vector[AMat2] PredAbundMatPac; //vector for population predictions, Pacific, (Matsouka)
    ...
    vector[AMat] loglikeAbundMatInd;
   ...
   for(I in Ibasin){
    ...
    //calculating q
    q = exp(mean(log(AEstMatInd./PredAbundMatInd)));
    //q[2] = exp(mean(log(AEstMatPac./PredAbundMatPac))); //Pacific
  ...
  //posterior predictive-- Abundance
    ...
    PostPredMatInd = lognormal_rng(log(q * PredAbundMatInd), sig[IndCVIdx]); //Indian 
